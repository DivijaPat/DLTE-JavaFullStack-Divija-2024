...
public class UserDetailsFileRepository implements UserDetailsRepository {
    ...
    public UserDetailsFileRepository(String url) {
        ...
        try {
            ...
            FileHandler fileHandler = new FileHandler("inputDetail.txt",true);
    ...
    public void addUsers() {
        accountList.add(new  Account("user1", "password1", "user1@example.com", 1234567890L, 1000.00D));
        accountList.add(new  Account("user2", "password2", "user2@example.com", 9876543210L, 2000.00D));
        accountList.add(new  Account("user3", "password3", "user3@example.com", 5555555555L, 3000.00D));
        writeIntoFile();
    }
    @Override
    ...
    public void updateAccounts(List<Account> accounts) throws IOException {
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("inputDetail.txt"))) {
            for (Account account : accounts) {
...
//        receiver.addToTransactionHistory(transactionInfo);
        System.out.println(sender.getUsername()+" has transferred amount "+amount+" to "+receiver.getUsername());
